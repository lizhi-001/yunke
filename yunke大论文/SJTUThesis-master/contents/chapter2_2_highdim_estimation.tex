...
基于不同的参数结构假设，高维 VAR 模型的估计方法大体可分为三类：作为基准的最小二乘估计、基于稀疏性假设的 Lasso 正则化估计、以及基于低秩结构假设的奇异值分解截断估计。本节依次介绍这三类方法的数学形式与理论性质。
...
为缓解高维 VAR 模型中的参数爆炸问题，一个自然思路是对系数矩阵引入稀疏性假设。该假设认为，虽然系统中的变量数量可能较多，但真正具有直接动态影响关系的变量对通常只占较小比例，因而真实参数矩阵中包含大量为零的元素。
...
然而，Lasso 的 $L_1$ 惩罚项对所有系数施加连续收缩，这一机制在压缩零元素的同时，也不可避免地将真正非零的大系数向零方向拉偏，造成系统性低估。更深层的问题在于：$L_1$ 范数在零点不可微，使得 Lasso 估计量不具备标准的渐近正态分布，其抽样分布随惩罚系数与真实稀疏结构的不同而呈现高度不规则的形态。这意味着，直接将 Lasso 估计代入似然比统计量时，残差协方差矩阵的估计精度受到偏差污染，检验统计量的分布也因此偏离其名义临界值所对应的理论分布。为绕开这一推断困难，本文采用基于残差重抽样的自举法（Bootstrap），在无需推导 Lasso 估计量解析分布的前提下，直接通过数值模拟逼近检验统计量在有限样本下的经验分布，从而实现对检验临界值的数据驱动校正。
